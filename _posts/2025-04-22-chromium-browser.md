...
그래서 이 글에서는 멀티 프로세스, 멀티 스레드 같은 개념 자체를 깊이 다루기보다는, **크롬 브라우저 내부 구성 및 통신 방식**과 **탭 간 데이터 공유**가 어떻게 이루어지는지 알아보겠습니다.
...
이러한 문제들을 해결하기 위해 Chromium은 **멀티 프로세스 아키텍처**와 고성능 JavaScript 엔진(V8) 등의 기술을 도입하며 지금의 크롬 아키텍처의 기반을 마련하게 됩니다.
...
> **샌드박스 (Sandbox)**
>
> 샌드박스란 소프트웨어 개발에서 사용되는 용어로, 격리된 환경을 의미해요. 이렇게 격리된 샌드박스 환경은 시스템의 운영 체제, 설치 프로그램 등 외부 영향을 주거나 받지 않아요. 프로그램이나 코드를 실행하고 테스트할 수 있는 공간
>
...
1. Origin A의 탭 1에서 `localStorage.setItem()`을 호출합니다.
