...
이 방법은 RN에서 제공하는 NativeModules의 HotUpdate 방식으로 동작하며, 서버 구축 혹은 프레임워크로의 전환 등 큰 개발 비용이 발생하지 않는 장점이 있습니다.

혼자 앱 개발을 담당하는 상황에서 다른 대안에 비해 개발 비용이 크게 소요되지 않는 점은 저희 팀에게 매력적인 선택지로 다가와 3번 대안을 선택하게 되었습니다.
...
추가로 이 워크플로우에서 한 가지 고려해야 할 점이 있었습니다. 바로 스토어 제출과 외부 저장소의 버전 업데이트 사이에 앱을 다운로드 받는 사용자가 최신 버전을 보장할 수 있도록 하는 것이였습니다.

![App Timeline Problem](assets/img/writing/14/timeline_problem.png){: width="640" }

기존 사용자와, 신규 사용자 2의 경우에는 최신 버전을 보장할 수 있지만, 문제는 1번과 2번 시간 사이에 앱을 다운로드 받는 경우에 발생합니다.
...
1. 신규 사용자의 번들 초기화 문제
  - 신규 사용자가 앱을 설치할 때, 스토어 배포 시 포함된 번들 버전과 외부 저장소에서 관리하는 최신 번들 버전 간의 불일치가 발생할 수 있습니다.
  - 이로 인해, 신규 사용자가 스토어 출시 이후 최신 번들을 받아야 하는 상황에서 정확한 동기화를 보장하기 어려운 문제가 존재합니다.
2. 버전별 Hotfix 처리의 어려움
  - 특정 버전에서 치명적인 버그가 발생했을 경우, 해당 버전 사용자만을 위한 Hotfix 업데이트를 적용하는 것이 어려운 문제가 있습니다.
3. 점진적 업데이트의 부재
  - 현재 방식에서는 모든 사용자가 항상 최신 버전을 강제 적용받는 구조입니다.
  - 하지만, 최신 버전에 예기치 못한 에러가 발생할 경우, 전체 사용자에게 영향을 미치는 심각한 문제가 발생할 가능성이 있습니다.
  - 이를 방지하기 위해 **점진적 업데이트(Gradual Rollout) 기법**을 적용해야 하지만, 현재 시스템에서는 이를 지원하지 못하고 있습니다.
